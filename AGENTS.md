...
- The overlay draws from `SwitcherTokens` in `src/theme.rs`: semantic colors authored in OKLCH, with neutrals tinted toward the user's accent hue and every text token a real color rather than an alpha of another. Contrast lives in the lightness gap, so a readability fix moves L and keeps chroma and hue. Edges are 1pt rings in pure black or white at low alpha, never palette grays, and separation comes from surfaces, not lines. Key names shown to the user come from `src/macos/shortcuts.rs`, which also feeds the Shortcuts settings tab.
