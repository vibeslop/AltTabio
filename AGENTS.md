...
- Releases carry the certificate pinned in `scripts/mac/release-certificate.sha1`; macOS ties every user's Accessibility and Screen Recording grants to it. Never replace the pin, create another release certificate, or overwrite the `release` environment's secrets: users would have to grant both permissions again. Its key lives only in those secrets and the maintainers' vault; releases come from the `release-macos` workflow on a `v*` tag.
