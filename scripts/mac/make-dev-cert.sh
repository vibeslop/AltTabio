#!/bin/zsh
# Creates a self-signed "AltTabio Dev" code-signing certificate in the login keychain.
#
# macOS remembers Accessibility and Screen Recording grants per code signature. A stable
# certificate keeps them across rebuilds, so the permissions only have to be granted once.
set -euo pipefail

if security find-identity -v -p codesigning 2>/dev/null | grep -q '"AltTabio Dev"'; then
    echo "The 'AltTabio Dev' certificate already exists"
    exit 0
fi
...
openssl pkcs12 -export -legacy -inkey "$work/key.pem" -in "$work/cert.pem" -name "AltTabio Dev" \
    -passout pass:alttabio -out "$work/cert.p12" >/dev/null 2>&1
...
echo "Created the 'AltTabio Dev' certificate; scripts/mac/build-app.sh uses it from now on"
